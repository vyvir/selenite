...
class FilesWindow(QtWidgets.QWidget):
    def __init__(self, files_list=""):
        ...
        self.calculate_total_and_to_download()

        self.text = QtWidgets.QLabel(f"{(len(self.files_list))} files / total / to download", alignment=QtCore.Qt.AlignCenter)
    ...
    def eventFilter(self, source, event):
        if event.type() == QtCore.QEvent.ContextMenu and source is self.tablewidget:
            ...
            mn_items = [("fm", "folder", "&Open in File Manager", self.open_fm, "Ctrl+f"),
                    ("sl", "slade", "&Open in Slade", self.files_data, "Ctrl+s"),
                    ("so", "search", "&Search online", self.files_data, "Ctrl+b")]
    ...
    def open_fm(self):
        ...
        #except TypeError:
        #    self.warn("No file specified.")
        #subprocess.run(f"xdg-open {}")
    ...
    @QtCore.Slot()
    def calculate_total_and_to_download(self):
        ...
        for item in self.files_list:
            for key in list(item):
                match key:
                    case 'size':
                        files_left_data.append(item[key][:-4])
                    #case 'installed':
                    #    if key == "False":
                    ##        files_left_data.append(item['size'])
        print(files_left_data)
    ...
    def cur_row(self, row):
        return self.files_list[self.tablewidget.currentRow()][row]
    ...
    def create_tablewidget(self):
        ...
        colcnt = len(self.files_list[0])
        rowcnt = len(self.files_list)
        ...
        self.tablewidget.setHorizontalHeaderLabels(["Folder", "Size", "Name", "MD5", "Downloadable", "Installed"])

        #print(len(self.tuples_list[0]))
        #print(len(self.dict_list))
        for i, (folder, size, name, md5, downloadable, installed) in enumerate(self.files_list):
                for j in range(colcnt):
                    item = QtWidgets.QTableWidgetItem(self.files_list[i][j])
